...
/*******************************************************************************
*	Added by vajih:                                                        *
*	This function revereses the character bitmap.                          *
*******************************************************************************/

static __inline  unsigned short mirror_char_u_16( unsigned short c ){
  short int result = 0;
  int i;
...
static __inline  unsigned short mirror_char_u_8( unsigned char c ) {
  short int result = 0;
  int i;
...
void GLCD_DrawChar_U8 (unsigned int x, unsigned int y, unsigned int cw, unsigned int ch, unsigned char *c) {
  ...
  for (j = 0; j < ch; j++) {
    ...
    for (i = 0; i <= cw-1; i++) {
#endif
      if((mirror_char_u_8(c[idx]) & (1 << i)) == 0x00) {
...
void GLCD_DrawChar_U16 (unsigned int x, unsigned int y, unsigned int cw, unsigned int ch, unsigned short *c) {
...
#if (HORIZONTAL == 1)
  //x = WIDTH-x-cw; //I have changed it write left to right
  GLCD_SetWindow(y, x, ch, cw);
#else
  GLCD_SetWindow(x, y, cw, ch);
#endif
  ...
  for (j = 0; j < ch; j++) {
    ...
    for (i = 0; i <= cw-1; i++) {
#endif
      if(( mirror_char_u_16( c[idx]) & (1 << i)) == 0x00) {
        wr_dat_only(BackColor);
      } else {
        wr_dat_only(TextColor);
      }
