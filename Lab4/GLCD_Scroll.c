...
// The chach size is CACHE_LINE_CAP lines of LCD_WIDTH + 1 length
// - Each line is an string, and string ends up to '0'.
uint8_t chache[CACHE_LINE_CAP][LCD_WIDTH + 1];
...
__inline uint32_t last_line() {
	return (cache_start + cache_size) % CACHE_LINE_CAP;
}

__inline uint32_t last_window_line() {
...
void print_string(uint8_t *str_ptr) {
	...
	print_text(str_ptr, strlen(str_ptr));
